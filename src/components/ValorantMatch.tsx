...
export async function ValorantMatch(props: any) {
    ...
    const agent = await getValorantAgents(playerData.characterId);
    return (
        <>
            <div
            ...
            >
                ...
                <div className="flex items-center">
                    <div className="h-full w-full overflow-hidden flex items-center">
                        <Image alt="Agent icon" width={175} height={175} src={agent.data.displayIcon} />
                    </div>
                </div>
            </div>
        </>
...
async function getValorantMatchData(matchId: string) {
    const res = await fetch(`https://api.jackli.dev/valorant/match-details/${matchId}`, { next: { revalidate: 1800 } });
    if (!res.ok) {
...
async function getValorantAgents(uuid: string) {
